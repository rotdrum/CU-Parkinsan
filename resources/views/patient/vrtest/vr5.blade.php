...
<a-scene >
        ...
        <a-text position="-1 1 -2" rotation="0 0 0" color="#000"   id="str" value="Pinch the right hand."></a-text>
    ...
    if (hand){
      ...
      if(dot <= vel_exercise){
        ...
        Swal.fire({
            ...
            html: 'คลิกเพื่อทำแบบทดสอบท่าต่อไป :)<br/><form method="POST" action="{{ route("updatevrmode5") }}" >@csrf<input type="hidden" name="id" value="{{ $id }}"><br/><input type="hidden" name="value_" value="{{ $value }}"><br/><input type="submit" value="Next" class="btn btn-primary"></form>',
        })
      }
    }
}
